...
with tabs[1]:
    ...
    with calc_tab1:
        ...
        p1.metric("Odds", horse_row["Odds"])
        p2.metric("Implied Probability", f"{odds_to_probability_percent(horse_row['Odds'])}%")
        p3.metric("Estimated Profit", f"${payout['Estimated Profit']:,.2f}")
        p4.metric("Estimated Return", f"${payout['Estimated Return']:,.2f}")
