...
with tabs[1]:
    ...
    bet_type = st.selectbox("Bet Type", ["Win", "Exacta Box", "Trifecta Box", "Superfecta Box"])
    selected = st.multiselect("Select Horses", ranked["Horse"].tolist(), default=ranked["Horse"].tolist()[:3])

    if bet_type == "Win" and len(selected) == 1:
        row = ranked[ranked["Horse"] == selected[0]].iloc[0]
        profit = base_bet * parse_fractional_odds(row["Odds"])
        st.write(f"Estimated profit: **${profit:,.2f}**")
        st.write(f"Estimated return: **${profit + base_bet:,.2f}**")
    elif bet_type in {"Exacta Box", "Trifecta Box", "Superfecta Box"}:
        needed = {"Exacta Box": 2, "Trifecta Box": 3, "Superfecta Box": 4}[bet_type]
        if len(selected) < needed:
            st.warning(f"{bet_type} requires at least {needed} horses.")
        else:
            if bet_type == "Exacta Box":
                combos = len(selected) * (len(selected) - 1)
            elif bet_type == "Trifecta Box":
                combos = len(selected) * (len(selected) - 1) * (len(selected) - 2)
            else:
                combos = len(selected) * (len(selected) - 1) * (len(selected) - 2) * (len(selected) - 3)
            st.metric("Combinations", combos)
            st.metric("Ticket Cost", f"${combos * base_bet:,.2f}")
